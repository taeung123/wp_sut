@extends('layouts.app')
@section('content')
    <div class="banner-event fixheight" style="background-image: url('{!! pll_current_language('slug') === 'vi' ? get_option('banner_event_vi') : get_option('banner_event') !!}');">
                            ...
                            <div class="calendar-event">
                                <div class="calendar">
                                    ...
                                    <div class="group calendar-header">
                                        <p class="pointer center monthname">{!! pll_current_language('slug') === 'vi' ? 'Sự kiện trước' : 'View past event' !!}</p>
                                    ...
                                    <div class="group calendar-header calendar-footer">
                                        <p> {!! pll_current_language('slug') === 'vi' ? 'Sự kiện trước' : 'View past event' !!}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row">
                                    ...
                                    @if ($single_post->have_posts())
                                        @while ($single_post->have_posts())
                                            ...
                                            @endphp
                                            @php
                                                $slip_date = explode('/', get_field('date_start', get_the_ID()));
                                                $slip_date_end = explode('/', get_field('date_end', get_the_ID()));
                                            @endphp
                                            <div class="col-lg-12">
                                                <div class="big-picture">
                                                    <div class="month-day monday-event">
                                                        <p class="have-border">
                                                            {{ $slip_date['2'] }}<br>{{ $slip_date['1'] }}</p>
                                                        <span></span>
                                                        <p>{{ $slip_date_end['2'] }}<br>{{ $slip_date_end['1'] }}</p>
                                                    </div>
                                                    <a href="{{ get_the_permalink() }}">
                                                        <img src="{{ get_the_post_thumbnail_url() }}"
                                                            alt="{{ get_the_permalink() }}" class="img-event"
                                                            style="background-image: url('{{ get_the_post_thumbnail_url() }}')">
                                                        <!-- {{ get_the_post_thumbnail_url() }} -->
                                                    </a>
                                                    <div class="tex-event-course">
                                                        <h2><a href="{{ get_the_permalink() }}">{{ get_the_title() }}
                                                            </a></h2>
                                                        {!! wp_trim_words(get_the_content(), $num_words = 15) !!}
                                                        <div class="go-to-link">
                                                            <a href="{{ get_the_permalink() }}">{!! pll_current_language('slug') === 'vi' ? 'khám phá' : 'discover' !!}<i
                                    ...
                                    @if ($query_event->have_posts())
                                        @while ($query_event->have_posts())
                                            ...
                                            @php
                                                ...
                                                $slip_date_end = explode('/', get_field('date_end', get_the_ID()));
                                            @endphp
                                            <div class="col-lg-4 set_attribute">
                                                <div class="big-picture d-flex justify-content-between flex-column">
                                                    <div class="div">
                                                        <div class="month-day">
                                                            <p>{{ $slip_date['2'] }}<br>{{ $slip_date['1'] }}</p>
                                                        </div>
                                                        <a href="{{ get_the_permalink() }}">
